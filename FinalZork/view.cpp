...
void View::setWidgetPos()
{
    ...
    healthy_face = new QMovie(":/images/healthy.gif");
    damage_face = new QMovie(":/images/bloody.gif");
    ...
    enemy_image->start();
    player_label->setGeometry(450, 450, 100, 100);
    player_label->setMovie(healthy_face);
    healthy_face->start();
...
{
    if(room_item_display->currentIndex().row() != -1) // will crash otherwise if don't do a check
...
void View::updateHealth() {
    player_label->setMovie(damage_face);
    healthy_face->stop();
    damage_face->start();
}
